...
curl -X PUT -H "Content-Type:application/json" -u $CM_USER:$CM_PASS $INSECURE \
  -d "{ \"items\": [ { \"name\": \"ssl_cacerts\", \"value\": \"$CERT_DIR/allserver.pem\" }, { \"name\": \"ssl_private_key_password\", \"value\": \"$KEYSTORE_PASS\" }, { \"name\": \"ssl_private_key\", \"value\": \"$CERT_DIR/server.key\" }, { \"name\": \"ssl_certificate\", \"value\": \"$CERT_DIR/server.pem\" } ] }" \
